...
# Always mount the static route; files may be populated later into the container.
try:
...
def verify_credentials(credentials: HTTPBasicCredentials = Depends(security), session: Session = Depends(get_session)):
...
def create_job(req: JobCreate, force: bool = False, lang: str = "de", username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
    ...
    try:
        ...
        if profile_name:
            family = resolve_compression_family(mime_type=input_obj.get("mime_type"), file_name_or_ext=file_name_or_ext)
            try:
                prof = get_compression_profile(family, profile_name)
            except KeyError:
                raise HTTPException(status_code=400, detail=f"Unknown compression profile '{profile_name}' for family '{family}'")
            ...
            if warning and not force:
                ...
                raise HTTPException(status_code=409, detail={"warning": warning, "message": "Use ?force=true to override"})
                ...
                try:
                    ...
                    audit_override(payload=req.dict(), username=username, profile=profile_name, lang=effective_lang)
                except Exception:
                    logger.exception("Failed to write override audit log")
                try:
                    # also create a DB-backed audit entry
                    # `session` is available via Depends
                    crud.create_audit_entry(session, username=username, profile=profile_name, payload=req.dict(), lang=effective_lang)
                except Exception:
    ...
    try:
        # Dispatch the worker pipeline task to download and convert the input
    ...
    return JobRead(id=job.id, type=job.type, status=job.status, progress=job.progress, current_step=job.current_step, output_path=job.output_path)


@app.get("/api/jobs/{job_id}")
def get_job(job_id: int, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
...
def job_events(job_id: int, request: Request, username: str = Depends(verify_credentials)):
...
@app.get("/api/jobs")
def list_jobs(limit: int = 50, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
...
def api_get_profile(family: str | None = None, profile: str = "balanced", lang: str = "de"):
    try:
...
@app.get("/api/audit")
def api_list_audit(limit: int = 100, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
...
@app.post("/api/flows")
def create_flow_endpoint(flow: FlowCreate, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
    f = crud.create_flow(session, flow.name, flow.steps, description=None)
    return {"id": f.id, "name": f.name, "steps": f.steps}


@app.get("/api/flows")
def list_flows_endpoint(username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
    fs = crud.list_flows(session)
    return fs


@app.get("/api/flows/{flow_id}")
def get_flow_endpoint(flow_id: int, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
...
def run_flow_endpoint(flow_id: int, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
...
def list_flow_runs_endpoint(flow_id: int, limit: int = 50, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
    runs = crud.list_flow_runs(session, flow_id, limit=limit)
    return runs


@app.get("/api/runs/{run_id}")
def get_run_endpoint(run_id: int, username: str = Depends(verify_credentials), session: Session = Depends(get_session)):
...
def run_events(run_id: int, request: Request, username: str = Depends(verify_credentials)):
...
def flow_events(flow_id: int, request: Request, username: str = Depends(verify_credentials)):
